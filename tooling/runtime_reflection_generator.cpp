...
void metapp::RuntimeReflectionGenerator::generate(
    ASTContext *ctx, raw_ostream &os,
  ...
  classFlags.hasBeforeSerialize =
      HasFunctionOfName("BeforeSerialize", reflectedClass.m_functions);
  classFlags.hasAfterSerialize =
      HasFunctionOfName("AfterSerialize", reflectedClass.m_functions);
  classFlags.hasCustomSerialize =
      HasFunctionOfName("Serialize", reflectedClass.m_functions);
  classFlags.hasCustomDump =
      HasFunctionOfName("Dump", reflectedClass.m_functions);
  ...
  os << "template<>\n"
     ...
     << "{\n"
     << "static detail::ClassStorage<" << type << ", "
     << reflectedClass.m_fields.size() << ", "
  ...
  for (unsigned i = 0, n = reflectedClass.m_fields.size(); i < n; ++i)
    fieldGenerator.Generate(i, reflectedClass.m_fields[i],
                            propertyAnnotations[i], os);

  /* Functions */
  FunctionGenerator funcGenerator(ctx, type, "self->functions");
  for (unsigned i = 0, n = reflectedClass.m_functions.size(); i < n; ++i)
    funcGenerator.Generate(i, reflectedClass.m_functions[i],
                           functionAnnotations[i], os);
